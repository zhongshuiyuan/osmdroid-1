...
public class DistancePolyline extends OverlayWithIW {
     ...
     */
    public DistancePolyline(MapView mapView) {
        if (mapView != null) {
     ...
     * Warning: it takes effect only if set before setting the points in the DistancePolyline.
     */
    ...
    public void setOnClickListener(DistancePolyline.OnClickListener listener) {
    ...
    public void draw(final Canvas canvas, final Projection pj) {
        ...
        int size = mOriginalPoints.size();

//        Paint paint = new Paint();
//        paint.setFlags(Paint.ANTI_ALIAS_FLAG);
//        paint.setTextSize(textSize);
//        paint.setColor(Color.BLACK);
//        GeoPoint point = mOriginalPoints.get(size - 1);
//        long x1 = mapView.getProjection().getLongPixelXFromLongitude(point.getLongitude());
//        long y1 = mapView.getProjection().getLongPixelYFromLatitude(point.getLatitude());

        double totalDist = 0;
        for (int j = 0; j < size - 1; j++) {
            GeoPoint point0 = mOriginalPoints.get(j);
            GeoPoint point1 = mOriginalPoints.get(j + 1);
            Paint paint = new Paint();
            paint.setFlags(Paint.ANTI_ALIAS_FLAG);
            paint.setTextSize(textSize);
            paint.setColor(Color.BLACK);
            long x1 = mapView.getProjection().getLongPixelXFromLongitude(point1.getLongitude());
            long y1 = mapView.getProjection().getLongPixelYFromLatitude(point1.getLatitude());
            double dist = GISUtils.getDistance(point0, point1);
            totalDist += dist;
            if (dist >= 1000) {
                canvas.drawText(m1(totalDist / 1000) + "公里", x1, y1, paint);
            } else {
                canvas.drawText(m1(totalDist) + "米", x1, y1, paint);
            }
        }
...
//        }
     ...
     * @return true if the DistancePolyline is close enough to the point.
     */
     ...
     * @param point
     * @param tolerance in pixels
     * @return the first GeoPoint of the DistancePolyline close enough to the point
     * @since 6.0.3
     * Detection is done is screen coordinates.
     */
...
//                return onClickDefault(this, mapView, closest);
    ...
    public boolean onLongPress(MotionEvent e, MapView mapView) {
        ...
        if (closest != null) {
            if (onlongClickListener != null) {
                return onlongClickListener.onLongClick(this, mapView, closest);
            }
        }
        return false;
    }
    ...
    public interface OnLongClickListener {
        abstract boolean onLongClick(DistancePolyline polyline, MapView mapView, GeoPoint eventPos);
    }
    ...
    //-- DistancePolyline events listener interfaces ------------------------------------

    public interface OnClickListener {
        abstract boolean onClick(DistancePolyline polyline, MapView mapView, GeoPoint eventPos);
    }

    /**
     * default behaviour when no click listener is set
     */
    public boolean onClickDefault(DistancePolyline polyline, MapView mapView, GeoPoint eventPos) {
