...
public class KmlPlacemark extends KmlFeature implements Cloneable, Parcelable {
    ...
    /**
     * constructs a Placemark from a TextMarker, as a KML Point
     */
    ...
    public KmlPlacemark(Polygon polygon, KmlDocument kmlDoc) {
        ...
        ((KmlPolygon) mGeometry).mHoles = (ArrayList) polygon.getHoles();
    ...
    public KmlPlacemark(JsonObject json) {
        ...
        if (json.has("projectId")) {
            mId = json.get("projectId").getAsString();
        }
    ...
    public JsonObject asGeoJSON(boolean isRoot) {
        ...
        if (mId != null) {
            json.addProperty("projectId", mId);
        }
        if (mGeometry != null) {
