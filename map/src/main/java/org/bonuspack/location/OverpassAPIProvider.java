package org.bonuspack.location;
...
import org.bonuspack.kml.KmlFolder;
import org.bonuspack.kml.KmlGeometry;
import org.bonuspack.kml.KmlLineString;
import org.bonuspack.kml.KmlMultiGeometry;
	...
	public ArrayList<POI> getPOIsFromUrl(String url){
		...
		try {
			...
			for (JsonElement j:jElements){
				...
				poi.mId = jo.get("projectId").getAsLong();
	...
	public boolean addInKmlFolder(KmlFolder kmlFolder, String url){
		...
		try {
			...
			for (JsonElement j:jElements){
				...
				placemark.mId = jo.get("projectId").getAsString();
				//Tags:
				if (jo.has("tags")){
					JsonObject jTags = jo.get("tags").getAsJsonObject();
					if (jTags.has("name")) {
