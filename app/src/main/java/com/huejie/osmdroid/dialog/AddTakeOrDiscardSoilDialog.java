 ...
 */
public class AddTakeOrDiscardSoilDialog extends BaseDialog {
    ...
    private AddTakeOrDiscardSoilDialog(Context context) {
    ...
    private static AddTakeOrDiscardSoilDialog dialog;

    public static AddTakeOrDiscardSoilDialog getInstance(Context context) {
        if (null == dialog) {
            dialog = new AddTakeOrDiscardSoilDialog(context);
        }
        return dialog;
    }
    ...
    public void onCreateDialog(Bundle savedInstanceState) {
        setContentView(R.layout.dialog_add_take_discard_soil);
        ...
        tv_confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (TextUtils.isEmpty(jczh)) {
                    ToastUtils.showShortToast("上路桩号不能为空");
                    return;
                }
                String name = et_name.getText().toString().trim();
                if (TextUtils.isEmpty(name)) {
                    ToastUtils.showShortToast("取（弃）土场名称不能为空");
                    return;
                }
                if (null != onClick) {
