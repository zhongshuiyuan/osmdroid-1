...
public class MySqlTileWriter implements IFilesystemCache, SplashScreenable {
    ...
    public static final String COLUMN_MAPTILEINDEX = "mapTileIndex";
    ...
    public MySqlTileWriter() {
        this(false, System.currentTimeMillis(), MapViewManager.getInstance().mMapView.getTileProvider().getTileSource().name());
    }
    ...
    public MySqlTileWriter(boolean isDownload, long tag, String mapType) {
        init();
    ...
    public boolean saveFile(final ITileSource pTileSourceInfo, final long pMapTileIndex, final InputStream pStream, final Long pExpirationTime) {
        ...
        if (isDownload) {
            ...
            File file = new File(Util.getOfflineMapPathByName(pTileSourceInfo.name(), pMapTileIndex));
            if (!FileUtils.createOrExistsFile(file)) {
                db = SQLiteDatabase.openOrCreateDatabase(file, null);
                db.execSQL(new StringBuilder().append("CREATE TABLE IF NOT EXISTS ").append(TABLE).append(" (").append(DatabaseFileArchive.COLUMN_KEY).append(" INTEGER , ").append(DatabaseFileArchive.COLUMN_PROVIDER).append(" TEXT, ").append(TAG).append(" LONG, ").append(DatabaseFileArchive.COLUMN_TILE).append(" BLOB, ").append(COLUMN_MAPTILEINDEX).append(" LONG, ").append(COLUMN_EXPIRES).append(" INTEGER, PRIMARY KEY (").append(DatabaseFileArchive.COLUMN_KEY).append(", ").append(DatabaseFileArchive.COLUMN_PROVIDER).append("));").toString());
                offlineMap.put(zoom, db);

            } else {
                //有离线地图文件
                if (null == offlineMap.get(zoom)) {
                    db = SQLiteDatabase.openOrCreateDatabase(file, null);
                    db.execSQL(new StringBuilder().append("CREATE TABLE IF NOT EXISTS ").append(TABLE).append(" (").append(DatabaseFileArchive.COLUMN_KEY).append(" INTEGER , ").append(DatabaseFileArchive.COLUMN_PROVIDER).append(" TEXT, ").append(TAG).append(" LONG, ").append(DatabaseFileArchive.COLUMN_TILE).append(" BLOB, ").append(COLUMN_MAPTILEINDEX).append(" LONG, ").append(COLUMN_EXPIRES).append(" INTEGER, PRIMARY KEY (").append(DatabaseFileArchive.COLUMN_KEY).append(", ").append(DatabaseFileArchive.COLUMN_PROVIDER).append("));").toString());
                    offlineMap.put(zoom, db);
                } else {
                    db = offlineMap.get(zoom);
                }
            }
        ...
        try {
            ...
            if (isDownload) {
                cv.put(COLUMN_MAPTILEINDEX, pMapTileIndex);
                cv.put(DatabaseFileArchive.COLUMN_PROVIDER, mapType);
                cv.put(TAG, tag);
            } else {
    ...
    public void onDetach() {
        synchronized (MySqlTileWriter.this) {
            if (null != offlineMap) {
                Iterator<SQLiteDatabase> iter = offlineMap.values().iterator();
                while (iter.hasNext()) {
                    SQLiteDatabase db = iter.next();
                    if (db.isOpen()) {
                        db.close();
                    }
                }
    ...
    /**
     * purges and deletes all tiles from the given tile source favoritesName from the cache database
     *
     * @return
     * @since 5.6.1
     */
    ...
    /**
     * Returns the number of tiles in the cache for the specified tile source favoritesName
     *
     ...
     * Count cache tiles
     *
     * @param pTileSourceName the tile source favoritesName (possibly null)
    ...
    public Cursor getTileCursor(final String[] pPrimaryKeyParameters, final String[] pColumns) {
        final SQLiteDatabase db = getDb();
        return db.query(DatabaseFileArchive.TABLE, pColumns, primaryKey, pPrimaryKeyParameters, null, null, null, "1");
    }
    ...
    private HashMap<Integer, SQLiteDatabase> offlineMap = new HashMap<>();
    ...
    public Drawable loadTile(final ITileSource pTileSource, final long pMapTileIndex) throws Exception {
        ...
        File file = new File(Util.getOfflineMapPathByName(pTileSource.name(), pMapTileIndex));
        if (file.exists()) {
            //有离线地图文件
            if (null == offlineMap.get(zoom)) {
                offlineDb = SQLiteDatabase.openOrCreateDatabase(file, null);
                offlineMap.put(zoom, offlineDb);
            } else {
                offlineDb = offlineMap.get(zoom);
            }

            cursor = offlineDb.query(DatabaseFileArchive.TABLE, queryColumns, primaryKey, getPrimaryKeyParameters(getIndex(pMapTileIndex), pTileSource), null, null, null);
            if (cursor.moveToFirst()) {
                bits = cursor.getBlob(cursor.getColumnIndex(DatabaseFileArchive.COLUMN_TILE));
                expirationTimestamp = cursor.getLong(cursor.getColumnIndex(SqlTileWriter.COLUMN_EXPIRES));
            }
            if (bits == null) {
                //如果离线地图里面没找到，就在缓存里面找
                final long index = getIndex(pMapTileIndex);
                cursor = getTileCursor(getPrimaryKeyParameters(index, pTileSource), queryColumns);
                if (cursor.moveToFirst()) {
                    bits = cursor.getBlob(cursor.getColumnIndex(DatabaseFileArchive.COLUMN_TILE));
                    expirationTimestamp = cursor.getLong(cursor.getColumnIndex(SqlTileWriter.COLUMN_EXPIRES));
                }
        ...
        synchronized (mLock) {
            ...
            if (mDb == null) {
                try {
                    ...
                    mDb.execSQL(new StringBuilder().append("CREATE TABLE IF NOT EXISTS ").append(TABLE).append(" (").append(DatabaseFileArchive.COLUMN_KEY).append(" INTEGER , ").append(DatabaseFileArchive.COLUMN_PROVIDER).append(" TEXT, ").append(DatabaseFileArchive.COLUMN_TILE).append(" BLOB, ").append(COLUMN_EXPIRES).append(" INTEGER, PRIMARY KEY (").append(DatabaseFileArchive.COLUMN_KEY).append(", ").append(DatabaseFileArchive.COLUMN_PROVIDER).append("));").toString());
                } catch (Exception ex) {
        ...
        synchronized (mLock) {
            if (mDb != null) {
                ...
                for (int i = 0; i < offlineMap.size(); i++) {
                    SQLiteDatabase db = offlineMap.get(i);
                    if (null != db) {
                        db.close();
                    }
                }
