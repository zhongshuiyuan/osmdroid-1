...
abstract class DataHandler extends LitePalBase {
    ...
    protected void putContentValuesForSave(LitePalSupport baseObj, Field field, ContentValues values)
        ...
        if (fieldValue != null) {
            ...
            String columnName;
            if (null != column && !TextUtils.isEmpty(column.columeName())) {
                columnName = column.columeName();
            } else {
                columnName = isIdColumn(field.getName()) ? "id" : DBUtility.convertToValidColumnName(field.getName());
            }

            Object[] parameters = new Object[]{changeCase(DBUtility.convertToValidColumnName(columnName)), fieldValue};
